	...
	<view>
		<view class="fab-box fab" :class="{
			...
			<view class="fab-circle" :class="{
			 ...
			 :style="{ 'background-color': styles.buttonColor }" @click="open">
				<text class="iconfont icontop" :class="{ active: showContent }"></text>
			</view>
			<view class="fab-content" :class="{
			 ...
			 :style="{ width: boxWidth, height: boxHeight, background: styles.backgroundColor }">
				...
				<view class="fab-item" v-for="(item, index) in content" :key="index" :class="{ active: showContent }" :style="{
						color: item.active ? styles.selectedColor : styles.color
					}"
				 @click="taps(index, item)">
					<text class="content-image iconfont f40 t-c" :class="[item.icon]"></text>
